    ...
    <div className="bg-gradient-to-r from-purple-900/40 to-slate-900/40 rounded-xl border border-purple-700/30 shadow-lg p-6 md:p-8 mb-8">
      <div className="flex items-center gap-3 mb-4">
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-purple-600/20">
          ...
          {title.toLowerCase().includes('security') && <Shield className="h-6 w-6 text-purple-400" />}
          {title.toLowerCase().includes('information') && <Users className="h-6 w-6 text-purple-400" />}
          {title.toLowerCase().includes('cookies') && <Clock className="h-6 w-6 text-purple-400" />}
          {title.toLowerCase().includes('contact') && <FileText className="h-6 w-6 text-purple-400" />}
          {/* Default icon */}
          {!['security','information','cookies','contact'].some(k=>title.toLowerCase().includes(k)) && <Shield className="h-6 w-6 text-purple-400" />}
        </span>
